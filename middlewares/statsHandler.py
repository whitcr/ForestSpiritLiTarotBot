...
class HandlerStatisticsMiddleware(BaseMiddleware):
    async def __call__(
        ...
        if isinstance(event, Message):
            ...
            command = self.process_callback_data(event.data)
        else:
            command = 'unknown_event'

        await self.update_statistics(command)
        return await handler(event, data)

    def process_callback_data(self, callback_data: str) -> str:
    ...
    async def update_statistics(self, command: str):
        now = datetime.utcnow()
        today = now.date()
