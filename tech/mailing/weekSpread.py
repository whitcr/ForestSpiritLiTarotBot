...
async def week_card_follow_schedule(bot):
    ids = await execute_select_all("select user_id from users where week_card_follow=1;")
    for user_id_tuple in ids:
        ...
        try:
            ...
            if is_booster or subscription:
                await get_week_spread_premium(user_id, bot, "недели")
            else:
                ...
                await execute_query("insert into spreads_week (user_id, file_id) values ($1, $2)", (user_id, file_id))
