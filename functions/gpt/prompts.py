TAROT_ASSISTANT_SYSTEM_PROMPT = (
    ...
    "трактовками простым и понятным языком. "
...
TAROT_READING_PROMPT = (
    ...
    "при их наличии) и после напиши общую трактовку на заданный вопрос"
)
...
DAILY_QUESTION_PROMPT = (
    ...
    "Также разбей этот вопрос на три подвопроса для трех позиций карт в триплете. Пример: Первая позиция: (конкретный "
    "вопрос,"
    ...
    "они раскрывали главный вопрос. Позиции также могут быть разнообразными, они не должны повторяться. Не добавляй "
    "ничего лишнего, только вопросы."
)
...
THEMED_TIME_SPREAD_PROMPT = (
    "Напиши трактовку расклада на картах таро Уэйта на тему {theme} на {time}. "
    ...
    " может себя чувствовать, приведи повседневные примеры того, как эти карты могут "
    "проиграться в заданной теме. Не более 500 символов."
)
