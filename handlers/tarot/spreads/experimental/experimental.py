...
async def send_triplet_image(bot, image, text, date, message, triplet_type: Union["mtriplet", "striplet"]):
    ...
    width, text_x, label_x = (3200, 1450, 1350) if triplet_type == "mtriplet" else (1770, 740, 655)

    if not text:
        draw_text.text((text_x, 70), 'МегаТриплет' if triplet_type == "mtriplet" else 'СуперТриплет', font = FONT_XL,
                       fill = 'white')
    else:
        current_h, pad = 70, 1910
        for line in textwrap.wrap(str(text), width = 100):
            w, h = text_size(line, FONT_XL)
            draw_text.text(((width - w) / 2, current_h), line, font = FONT_XL)
            current_h += h + pad

    draw_text.text((label_x, 1900), 'from @ForestSpiritLi', fill = 'white', font = FONT_XL)
    draw_text.text(((width - text_size(date, FONT_XL)[0]) / 2, 10), date, fill = 'white', font = FONT_XL)

    current_h, pad = 1950, 10
    for line in textwrap.wrap(f"for {message.from_user.first_name}", width = 30):
        w, h = text_size(line, FONT_S)
        draw_text.text(((width - w) / 2, current_h), line, font = FONT_L)
        current_h += h + pad
