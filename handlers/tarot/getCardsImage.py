...
from functions.createImage import get_choice_spread, get_random_num, get_path_cards, get_gradient_3d,\
...
async def set_card_image(message: types.Message, theme):
    try:
        ...
        await bot.send_sticker(message.chat.id, sticker = sticker_id, reply_to_message_id = message.message_id)
...
async def get_card_image_with_text(message: types.Message, theme, *num, choice=None):
    ...
    if choice == 'raider':
        ...
        text = execute_select(f"select {theme} from meaning_raider where number = {num};")
...
async def send_card_image(message, image):
    try:
        ...
        with bio:
            ...
            await bot.send_photo(message.chat.id, photo = bio, reply_to_message_id = reply_to_message_id)
