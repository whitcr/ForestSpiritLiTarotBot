...
from functions.createImage import get_choice_spread, get_path_cards
from functions.temporaryStore import store_data
from handlers.tarot.getMeaningCb import generate_meaning_keyboard, MEANINGS_BUTTONS
...
async def get_meaning(message: types.Message):
    if re.search(r'значение\Z', message.text.lower()):
        ...
        try:
            ...
            num = execute_select("select number from meaning_raider where name ='{}';".format(name.lower()))
            ...
            if choice in MEANINGS_BUTTONS.keys():
                meaning_text = execute_select("select general from {} where number = {};".format(table, num))
                ...
                await message.reply(
                    f"— К сожалению, пока что у меня нет значений на выбранную колоду. Значение есть только для Уэйта, Безумной Луны, Чеколли и Манары.")
                return
...
async def get_meaning_text(message: types.Message, state: FSMContext):
    try:
