...
from functions.temporaryStore import store_data, get_data_two_nums
...
@router.message(StateFilter(None), F.text.lower() == "узнать аркан")
...
async def get_arcan_date(message: types.Message, state: FSMContext):
    ...
    card1 = execute_select("select name from cards where number = {};".format(date_sum))

    card2 = execute_select("select name from cards where number = {};".format(arcane))
...
async def personal_date_arcane(call: types.CallbackQuery):
    await call.answer()
    if call.from_user.id == call.message.reply_to_message.from_user.id:
        num1, num2 = await get_data_two_nums(call.message.message_id)

        name = execute_select(f"SELECT name FROM meaning_raider WHERE number = {num1}")
        text = execute_select(f"SELECT arcane_personal_date FROM meaning_raider WHERE number = {num1}")
...
async def personal_fate_arcane(call: types.CallbackQuery):
    await call.answer()
    if call.from_user.id == call.message.reply_to_message.from_user.id:
        num1, num2 = await get_data_two_nums(call.message.message_id)

        name = execute_select(f"SELECT name FROM meaning_raider WHERE number = {num1}")
        text = execute_select(f"SELECT arcane_fate_date FROM meaning_raider WHERE number = {num1}")
